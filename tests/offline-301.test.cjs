...
function worker() {
  ...
  const state = {offline: false, networkCalls: 0, claimed: false, installed: false};
  ...
  const network = async request => {
    ...
    return new Response(fs.readFileSync(path.join(root, file), 'utf8'), {headers: {'content-type': type}});
  };
  const caches = {
    async open(name) {
...
test('3.0.1: precaché coincide exactamente con los scripts y estilos versionados', async () => {
  const sw = worker();
  ...
  assert.equal(sw.run('CACHE'), 'profeecep-3.0.1');
  assert.match(html, /name="profeecep-version" content="3.0.1"/);
});

test('3.0.1: todos los scripts cargan desde caché sin devolver HTML', async () => {
  const sw = worker();
  await sw.lifecycle('install');
  sw.state.offline = true;
  const calls = sw.state.networkCalls;
  for (const file of scripts) {
    const response = await sw.request(file + '?v=3.0.1');
...
test('3.0.1: HTML offline se limita a navegaciones; no intercepta API ni recursos desconocidos', async () => {
  const sw = worker();
  await sw.lifecycle('install');
  sw.state.offline = true;
  ...
  assert.equal(sw.request('https://another.test/data.js?v=3.0.1'), undefined);
  sw.stores.get(sw.run('CACHE')).delete(sw.key('release-30.js?v=3.0.1'));
  assert.equal((await sw.request('release-30.js?v=3.0.1')).type, 'error');
});

test('3.0.1: activa su caché y conserva recursos de otras aplicaciones', async () => {
  const sw = worker();
  await sw.caches.open('profeecep-3.0');
  await sw.caches.open('another-app');
  await sw.lifecycle('install');
  await sw.lifecycle('activate');
  assert.deepEqual(await sw.caches.keys(), ['another-app', 'profeecep-3.0.1']);
  assert.equal(sw.state.claimed, true);
});

test('3.0.1: todos los scripts clásicos tienen sintaxis válida', () => {
