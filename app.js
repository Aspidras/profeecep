...
function finishDiagnosis(){const byDomain={};answers.forEach(a=>{if(!byDomain[a.d])byDomain[a.d]={right:0,total:0};byDomain[a.d].total++;if(a.ok)byDomain[a.d].right++});S.diagnosis={date:new Date().toISOString(),right:answers.filter(a=>a.ok).length,total:answers.length,byDomain};saveState();render();showDiagnosisResult()}
