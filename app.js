...
function render(){
 ...
 mapEl.innerHTML='<h2>Mapa ECEP 2026</h2><p class="muted">Abre un indicador para estudiarlo o márcalo como revisado.</p>'+D.map((d,di)=>'<div class="domain"><div class="row"><h3>'+d[0]+'</h3><button class="btn ghost" onclick="startQuiz(\''+d[0]+'\',5)">Practicar</button></div>'+d[1].map((s,si)=>'<div class="sub"><b>'+s[0]+'</b>'+s[1].map((t,ii)=>{let id=di+"-"+si+"-"+ii;return '<div class="indicator"><button type="button" class="indicatorText" onclick="openStudy(\''+id+'\')"><span class="studyIcon">📚</span><span>'+t+'</span></button><div class="indicatorActions"><button class="mini ghost2" onclick="openStudy(\''+id+'\')">Estudiar</button><button class="mini '+(S.done.includes(id)?"done":"")+'" onclick="toggle(\''+id+'\')">'+(S.done.includes(id)?"✓":"○")+'</button></div></div>'}).join("")+'</div>').join("")+'</div>').join("");
...
function showQ(){if(pos>=quiz.length)return mode==="diagnosis"?finishDiagnosis():(mode==="simulation"?finishSimulation():finishPractice());sel=null;const x=quiz[pos],target=mode==="diagnosis"?el("diagnosis"):(mode==="simulation"?el("simulation"):el("practice"));target.innerHTML='<div class="row"><span class="pill">'+(mode==="diagnosis"?"Diagnóstico · ":"")+x.d+'</span><b>'+(pos+1)+' / '+quiz.length+'</b></div><div class="bar stepbar"><i style="width:'+Math.round(pos/quiz.length*100)+'%"></i></div><div class="card"><div class="muted">'+x.i+'</div><h2>'+PE304.esc(x.q)+'</h2>'+(window.PE305?PE305.stimulus(x):'')+x.o.map((o,i)=>'<button class="opt" onclick="pick('+i+',this)">'+String.fromCharCode(65+i)+'. '+PE304.esc(o)+'</button>').join("")+'<button class="btn full" onclick="checkAnswer()">Responder</button></div>';go(mode==="diagnosis"?"diagnosis":(mode==="simulation"?"simulation":"practice"))}
function pick(i,b){sel=i;document.querySelectorAll(".opt").forEach(x=>x.classList.remove("sel"));b.classList.add("sel")}
function checkAnswer(){if(sel===null||answers.length>pos)return;const x=quiz[pos],ok=sel===x.a;answers.push({id:x.id,d:x.d,ok,selected:sel});if(mode==="simulation"){pos++;showQ();return}if(mode==="practice"){S.total++;if(ok)S.right++;S.history.push({date:new Date().toISOString().slice(0,10),domain:x.d,id:x.id,ok,selected:sel});if(S.history.length>300)S.history=S.history.slice(-300);if(!S.byDomain[x.d])S.byDomain[x.d]={right:0,total:0};S.byDomain[x.d].total++;if(ok)S.byDomain[x.d].right++;if(!ok&&!S.wrong.includes(x.id))S.wrong.push(x.id);if(ok)S.wrong=S.wrong.filter(id=>id!==x.id);saveState()}const target=mode==="diagnosis"?el("diagnosis"):el("practice");target.innerHTML='<div class="card '+(ok?"good":"bad")+'"><span class="pill">'+(ok?"Correcta":"Revisar")+'</span><h2>'+(ok?"¡Bien!":"No era esa")+'</h2>'+PE304.feedback(x,sel)+'</div><button class="btn full" onclick="nextQ()">Siguiente</button>'}
function nextQ(){pos++;showQ()}
function finishPractice(){const r=answers.filter(a=>a.ok).length,p=pct(r,answers.length);el("practice").innerHTML='<h2>Sesión terminada</h2><div class="card"><span class="muted">Resultado</span><strong class="score">'+r+' / '+answers.length+'</strong><div class="bar"><i style="width:'+p+'%"></i></div><p>'+p+'% de respuestas correctas</p></div><h3>Revisión</h3>'+answers.map(a=>PE304.review(Q.find(q=>q.id===a.id),a.selected)).join("")+'<button class="btn full" onclick="render();go(\'practice\')">Volver a practicar</button>';saveState()}
...
function renderSimulation12(){
 ...
 '<div class="card sim12Question"><div class="muted">'+x.d+' · '+x.i+'</div><h2>'+PE304.esc(x.q)+'</h2>'+(window.PE305?PE305.stimulus(x):'')+
 x.o.map((o,i)=>'<button class="opt '+(selected===i?"sel":"")+'" onclick="sim12Choose('+i+')">'+String.fromCharCode(65+i)+'. '+PE304.esc(o)+'</button>').join("")+
...
function finishSimulation12(auto=false){
 ...
 rows.forEach(a=>{
   ...
   if(!a.unanswered){
     S.history.push({date:new Date().toISOString().slice(0,10),domain:a.d,id:a.id,ok:a.ok,selected:a.selected});
 ...
 S.simulations.push({sessionId:SIM12.sessionId,date:new Date().toISOString(),right:correct,total:quiz.length,byDomain:by,durationSeconds:maxSeconds-SIM12.seconds,unanswered,version:SIM12.generatedBy||"1.2"});
 ...
 '<h3>Revisión de respuestas</h3>'+rows.map(a=>PE304.review(Q.find(q=>q.id===a.id),a.selected)).join("")+
