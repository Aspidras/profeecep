...
(function(){
 ...
 function inventory(){
  const subs=[];let indicators=0;D.forEach(d=>d[1].forEach(s=>{subs.push({domain:d[0],name:s[0],count:s[1].length,questions:Q.filter(q=>q.i===s[0]).length});indicators+=s[1].length}));
  const domains=[...new Set(D.map(d=>d[0]))];return{domains,subs,indicators,questions:Q.length};
 }
 function startFull(){
