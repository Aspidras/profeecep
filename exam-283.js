...
(function(){
 ...
 function inject(){
  ...
  if(practice&&!practice.querySelector('.exam283')){const c=document.createElement('div');c.className='card exam283';const count=bp.reduce((sum,row)=>sum+row.target,0);c.innerHTML='<span class="pill">Simulacro de práctica 3.0.6</span><h3>Entrenamiento de '+count+' preguntas únicas</h3><p class="muted">Práctica de 150 minutos con preguntas de entrenamiento y adaptaciones. La distribución equilibra los dominios según el banco disponible; no representa ponderaciones oficiales.</p><button class="btn full" onclick="PE283.start()">Iniciar simulacro de '+count+' preguntas</button>';const first=practice.querySelector('.complete28practice')||practice.querySelector('.sim21');if(first)first.before(c);else practice.prepend(c)}
